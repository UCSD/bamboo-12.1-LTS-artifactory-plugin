...
public abstract class AbstractArtifactoryConfiguration extends AbstractTaskConfigurator implements
    ...
    private void populateContextForAllOperations(@NotNull Map<String, Object> context) {
        // Bamboo 12.1 FreeMarker security (GenericObjectModel) blocks method invocations on
        // classes/proxies not in its allowlist. Expose both server configs and credentials as
        // plain Maps so FreeMarker map-key access bypasses the restriction.
        List<Map<String, Object>> serverConfigMaps = new ArrayList<>();
        for (org.jfrog.bamboo.admin.ServerConfig sc : serverConfigManager.getAllServerConfigs()) {
            Map<String, Object> m = new LinkedHashMap<>();
            m.put("id", sc.getId());
            m.put("url", sc.getUrl() != null ? sc.getUrl() : "");
            serverConfigMaps.add(m);
        }
        context.put("allServerConfigMaps", serverConfigMaps);

        List<Map<String, Object>> credentialMaps = new ArrayList<>();
        if (credentialsAccessor != null) {
            try {
                for (com.atlassian.bamboo.credentials.CredentialsData cred : credentialsAccessor.getAllCredentials()) {
                    Map<String, Object> m = new LinkedHashMap<>();
                    m.put("name", cred.getName() != null ? cred.getName() : "");
                    credentialMaps.add(m);
                }
            } catch (Exception e) {
                log.warn("Failed to load credentials for task config form", e);
            }
        }
        context.put("allCredentialMaps", credentialMaps);
