...
[@ui.bambooSection titleKey='artifactory.task.resolutionConfigurationsTitle' collapsible=true]
    ...
    [@ui.bambooSection id="resolutionSection" dependsOn='resolveFromArtifacts' showOn=true ]
        [@ww.select name='builder.artifactoryMaven3Builder.resolutionArtifactoryServerId' labelKey='artifactory.task.maven.resolutionArtifactoryServerUrl' list=allServerConfigMaps listKey='id' listValue='url' onchange='javascript: displayResolutionMaven3ArtifactoryConfigs(this.value)' emptyOption=true toggle=true /]
        <div id="maven3ArtifactoryResolutionConfigDiv">
            ...
            [@ui.bambooSection dependsOn='resolver.overrideCredentialsChoice' showOn='sharedCredentials']
                [@ww.select name='resolver.sharedCredentials' labelKey='artifactory.task.sharedCredentials' list=allCredentialMaps
...
[@ui.bambooSection id="deploymentSection" titleKey='artifactory.task.deploymentConfigurationsTitle' collapsible=true]
    [@ww.select name='builder.artifactoryMaven3Builder.artifactoryServerId' labelKey='artifactory.task.maven.artifactoryServerUrl' list=allServerConfigMaps
    ...
    <div id="maven3ArtifactoryConfigDiv">
        ...
        [@ui.bambooSection dependsOn='deployer.overrideCredentialsChoice' showOn='sharedCredentials']
            [@ww.select name='deployer.sharedCredentials' labelKey='artifactory.task.sharedCredentials' list=allCredentialMaps
            listKey='name' listValue='name' toggle=true/]
        [/@ui.bambooSection]
...
[@ui.bambooSection titleKey='artifactory.task.buildInfoReleaseManagementTitle' collapsible=true]
    <div style="visibility:hidden; position:absolute; height:0; overflow:hidden;">
        [@ww.checkbox name='buildInfoAggregation' toggle=true/]
        </div>
...
<script>
    ...
    function displayRequiredFieldset() {
        if (document.getElementsByName("buildInfoAggregation").length > 0 && document.getElementsByName("buildInfoAggregation")[0].checked) {
            ...
            var _pub = document.getElementById("publishBuildInfoSet"); if (_pub) _pub.remove();
        } else {
            // This is an old task. Remove all fieldset that depends on the new task.
            var _cap = document.getElementById("captureBuildInfoSet"); if (_cap) _cap.remove();
        }
    }
    ...
    // Remove builder.xxx helper entries before Bamboo treats them as OGNL paths.
    document.querySelectorAll('input[name="checkBoxFields"], input[name="selectFields"]').forEach(function(el) {
        if (el.value && el.value.indexOf('builder.') === 0 && el.parentNode) {
            el.parentNode.removeChild(el);
        }
    });
</script>
[#include 'ensureTaskPlanKey.ftl'/]
